...
| Leader | - [ ] 01 | `leader-01-tab.png` | Leader Robot 탭 — 좌측 Start Leader Robot 버튼 + 우측 Stepper |
| Leader | - [ ] 02 | `leader-02-origin-mapping.png` | Origin Setting — DXL 카드 드래그 + Set as Gripper 버튼 |
...
| Home Return | - [ ] 01 | `home-01-toggle.png` | home 배지 — 파란색(ON) / 회색(OFF) 두 상태 |
...
| Buttons | - [ ] 01 | `btn-01-toolbar.png` | MonitoringWindow 툴바 — SUCCESS / DONE / STOP 3개 버튼 |
...
| 페이지 | # | 파일명 | 캡처 대상 |
|---|---|---|---|
| Common | - [ ] 00 | `context-menu.png` | **통합 컷** (edit-00 + augment-01 + export-01 + export-02 대체) — 데이터셋 우클릭 메뉴 전체 (Edit / Augment / Merge / Export / Delete 항목이 모두 보이는 한 컷) |
...
| Downsample | - [ ] 01 | `downsample-01-dialog.png` | 데이터셋 다운샘플 다이얼로그 — name / 유지 비율(keep) / N프레임마다 1프레임 유지(every) 입력 + 다운샘플 버튼 |
| Episode Edit | - [ ] 01 | `episode-edit-01-panel.png` | **통합 컷** (episode-edit-03-layout 흡수) — 데이터셋 탭에서 에피소드 클릭 시 우측 EpisodePanel 전체 + 섹션 카드 분리 + 좌우 패널 높이 동기화 + 카메라 수별 이미지 크기 |
| Episode Edit | - [ ] 02 | `episode-edit-02-trim.png` | Trim 슬라이더 + Apply 버튼 |
| Import / Export | - [ ] 03 | `export-03-save-dialog.png` | OS 네이티브 "다른 이름으로 저장" 다이얼로그 — 제안된 파일명 `<name>_<id>.tar.gz` 노출 |
| Import / Export | - [ ] 04 | `import-01-button.png` | 데이터셋 탭(또는 WorkspacePage data 탭) 상단의 "데이터셋 불러오기" 버튼 |
| Import / Export | - [ ] 05 | `import-02-file-picker.png` | OS 네이티브 파일 선택 다이얼로그 — `.tar.gz` 아카이브 선택 |
| Import / Export | - [ ] 06 | `import-03-listed.png` | 가져온 데이터셋이 데이터셋 목록에 새 행으로 추가된 상태 |
...
| Create+Workspaces | - [x] 02 | `create-02-new.png` | 드롭다운 안 "+ 새 플래너 만들기" 항목 |
| Create+Workspaces | - [ ] 03 | `create-03-form.png` | 새 플래너 만들기 다이얼로그 |
| Create+Workspaces | - [ ] 04 | `create-04-edit-delete.png` | 플래너 항목 우측 edit/delete 아이콘 |
| Create+Workspaces | - [x] 05 | `workspaces-01-add-button.png` | 좌측 패널 "워크스페이스 추가" 버튼 |
| Create+Workspaces | - [x] 06 | `workspaces-02-multiselect.png` | 워크스페이스 선택 다이얼로그 (다중 선택) |
| Create+Workspaces | - [x] 07 | `workspaces-03-expanded.png` | 워크스페이스 expansion 펼침 — 센서/로봇 토글 |
| Create+Workspaces | - [x] 08 | `workspaces-04-remove.png` | 워크스페이스 헤더 X 버튼 |
...
| Blocks+Run | - [ ] 03 | `blocks-03-name.png` | 블록 이름 입력 필드 |
| Blocks+Run | - [ ] 04 | `blocks-04-save.png` | 블록 다이얼로그 저장 버튼 |
