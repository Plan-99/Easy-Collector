...
| OMY | - [ ] 01 | `omy-01-hardware.png` | OMY-F3M 본체 + RH-P12-RN 그리퍼 + RealSense D405 (온보드 PC 운용) |
| OMY | - [ ] 02 | `omy-02-online.png` | OMY 카드 ONLINE + RobotPendant 조인트 슬라이더 (6-DOF + 그리퍼) |
| OMY | - [ ] 03 | `omy-03-ssh-fields.png` | 로봇 등록 화면의 SSH Host / SSH User / SSH Port / SSH 비밀번호 입력 필드 (원격 SSH 운용, 기본 경로) |
| AI Worker | - [ ] 01 | `ai_worker-01-hardware.png` | AI Worker 본체 — 듀얼 7-DOF 팔 + 그리퍼 (온보드 NVIDIA Orin PC) |
| AI Worker | - [ ] 02 | `ai_worker-02-ssh-fields.png` | 로봇 등록 화면의 SSH Host / SSH User(robotis) / SSH Port / SSH 비밀번호 입력 필드 |
| AI Worker | - [ ] 03 | `ai_worker-03-online.png` | AI Worker 카드 ONLINE + RobotPendant 조인트 슬라이더 (양팔 16-DOF) |
...
| Dataset Select | - [ ] 03 | `dataset-03-sample-weight.png` | 선택된 카드 내부의 샘플링 비중 토글 (×½ / ×1 / ×2 / ×3 / ×5) — 현재 값 라벨 포함 |
...
폴더: `public/docs/curriculum/`

| 페이지 | # | 파일명 | 캡처 대상 |
|---|---|---|---|
| Index | - [x] 02 | `02-overview.png` | 플래너 선택 후 전체 패널 — 좌측 플래너 탭 블록 리스트 + 우측 롤아웃 컨트롤 바 + 그룹 대시보드 카드 |
| Setup | - [x] 01 | `01-empty.png` | 플래너 미선택 빈 상태 — 헤더 "스스로 배우는 로봇" + 플래너 선택 + 안내 문구 |
| Setup | - [x] 03 | `03-tab-planner.png` | 좌측 "플래너" 탭 — 블록 리스트, 각 블록 톱니/그룹 칩 |
| Setup | - [x] 04 | `04-tab-device.png` | 좌측 "디바이스" 탭 — 로봇/센서, 토픽 켜짐/꺼짐 상태 |
| Setup | - [x] 05 | `05-tab-policy.png` | 좌측 "그룹 정책" 탭 — 그룹 선택 + Stage Mission 폼 (목표 성공+교정 / 실패 저장 확률 / 저장) |
| Setup | - [x] 06 | `06-block-settings-motion.png` | 모션 블록 설정 다이얼로그 — 영향 줄 그룹, 노이즈 rate/offset 6축, 예상 노이즈 프리뷰 |
| Setup | - [x] 07 | `07-block-settings-checkpoint.png` | 체크포인트 블록 학습 설정 다이얼로그 — Base 데이터셋, 학습 서버주소, 하이퍼파라미터, 판정 조건 |
| Run | - [ ] 08 | `run-start.png` | 롤아웃 컨트롤 바 — 타겟 그룹 선택 + 반복 횟수 + 시작 버튼 |
| Run | - [ ] 09 | `run-monitor.png` | 전체화면 모니터 — 라이브 카메라/로봇 뷰, 현재 블록, 스텝 진행바, 중단 버튼 |
| Run | - [ ] 10 | `run-correction.png` | "체크포인트 실패" 다이얼로그 — 예, 교정 시작 / 건너뛰기 |
| Run | - [ ] 11 | `run-correction-teleop.png` | 교정 텔레옵 설정 — 자연어 지시, 텔레오퍼레이션 타입, Hz, 수집 시작 |
| Run | - [ ] 12 | `run-dashboard.png` | 그룹 대시보드 카드 — 성공/실패/교정/평균 길이, 판정 조건, 노이즈 범위, 성공률 라인차트 |
