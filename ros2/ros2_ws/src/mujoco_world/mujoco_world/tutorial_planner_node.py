...
class TutorialPlannerNode(Node):
    def __init__(self) -> None:
        ...
        # 14 mm peg.
        # gripper_closed: the finger inner face contacts the 14 mm peg at
        # gripper≈0.005. Commanding target *below* that creates grip force
        # (F = kp*(0.005 - target)). target=0.000 over-travels 5 mm into the
        # peg — the contact solver then squeezes the rigid peg OUT from between
        # the fingers (watermelon-seed ejection) and the empty fingers close to
        # 0; the peg free-falls the instant motion stops. target=0.004 leaves a
        # gentle 1 mm over-travel: enough steady pinch to hold, little enough
        # that the peg stays captured. (=0.005 would be zero force → slips.)
        self.declare_parameter("gripper_open", 0.020)
        self.declare_parameter("gripper_closed", 0.004)
        # Total joint travel per waypoint hop is capped at this many radians/m
        # per step so the sim doesn't see step-function commands. Lower = slower
        # motion (speed ≈ max_step_rad × plan_hz rad/s). 0.02 @ 20 Hz ≈ 0.4 rad/s
        # — the DAgger expert moves gently so grasps/inserts aren't rushed.
        self.declare_parameter("max_step_rad", 0.01)
        ...
        self.create_service(
            ...
            callback_group=cb_group)
        # Phase-split experts for curriculum DAgger: cp_grasp corrections call
        # run_grasp (grasp+lift only, peg left held); cp_insert corrections call
        # run_insert (over_hole→insert→release from the grasped state). Neither
        # randomizes — the rollout owns the scene wv1/wv2 detect against. Keeping
        # them on separate checkpoints means grasp vs insert dagger data lands in
        # separate datasets and trains independently.
        self.create_service(
            Trigger, f"{self._prefix}/run_grasp", self._on_run_grasp,
            callback_group=cb_group)
        self.create_service(
            Trigger, f"{self._prefix}/run_insert", self._on_run_insert,
            callback_group=cb_group)
    ...
    def _run_phase(self, response: Trigger.Response, phase: str,
                   randomize: bool) -> Trigger.Response:
        with self._lock:
            if self._busy:
            ...
            self._busy = True
        try:
            ok, msg = self._run_one_episode(phase=phase, randomize=randomize)
    ...
    def _on_run_episode(self, request: Trigger.Request,
        ...
        return self._run_phase(response, phase='full', randomize=True)

    def _on_run_grasp(self, request: Trigger.Request,
                       response: Trigger.Response) -> Trigger.Response:
        # cp_grasp DAgger expert: grasp the peg at its current pose, leave held.
        return self._run_phase(response, phase='grasp', randomize=False)

    def _on_run_insert(self, request: Trigger.Request,
                        response: Trigger.Response) -> Trigger.Response:
        # cp_insert DAgger expert: insert the already-grasped peg into the hole.
        return self._run_phase(response, phase='insert', randomize=False)
    ...
    def _run_one_episode(self, phase: str = 'full', randomize: bool = True) -> tuple[bool, str]:
        """Execute the peg-in-hole planner.

        ``phase``:
          'full'   — randomize + grasp + insert (one full episode, default).
          'grasp'  — grasp + lift only. Used as the cp_grasp DAgger expert:
                     the model failed to grasp, so the planner grasps the peg
                     and leaves it held; the rollout then continues to wv2 /
                     cp_insert. Does NOT randomize (the scene wv1 just detected
                     must stay put) and does NOT insert.
          'insert' — over_hole + insert + release only, from the already-grasped
                     state. Used as the cp_insert DAgger expert.
        Corrections (grasp/insert) never randomize — the rollout owns the scene.
        """
        ...
        if randomize and self._randomize_client.wait_for_service(timeout_sec=2.0):
        ...
        grasp_offset = 0.010     # grasp 10 mm above peg origin — higher so the EE
                                 # doesn't descend too low and knock the peg over
                                 # (fingers still wrap the upper-mid of the 60 mm peg)
        ...
        if phase == 'grasp':
            ...
            # cp_insert expert: pure insert of the already-grasped peg. If the
            # peg isn't currently held (grip slipped → dropped back to rest),
            # FAIL fast WITHOUT inserting — the episode is then discarded so the
            # cp_insert dagger dataset stays pure (insert-from-grasped only).
            peg_lifted = float(peg_xyz[2]) > 0.06
            if not peg_lifted:
                return False, (f"insert_skipped: peg not held "
                               f"(peg_z={peg_xyz[2]:.3f} ≤ 0.06) — no data collected")
            waypoints = insert_wps
        else:
            waypoints = grasp_wps + insert_wps
        ...
        time.sleep(0.4)
        if phase == 'grasp':
            # grasp success = peg lifted off the plate and still in the gripper.
            # The full peg-in-hole check_success doesn't apply mid-task, so we
            # judge on the peg having risen above its starting rest height.
            with self._lock:
                peg_now = (self._latest_poses or {}).get("peg")
            if peg_now is None:
                return False, "grasp_fail: peg pose unavailable"
            lifted = float(peg_now[2]) - float(peg_xyz[2])
            min_lift = 0.5 * approach
            if lifted < min_lift:
                return False, (f"grasp_fail: peg not lifted "
                               f"(Δz={lifted*1000:.1f}mm < {min_lift*1000:.1f}mm)")
            return True, f"grasp complete; peg lifted Δz={lifted*1000:.1f}mm"
        # insert / full → ground-truth peg-in-hole check.
