#!/usr/bin/env python3
#
# EasyTrainer launch wrapper for OpenManipulator-X Follower (omx_f).
#
# Differences from the upstream omx_f.launch.py:
#   - Spawns a single `joint_group_position_controller` (Float64MultiArray
#     over /joint_group_position_controller/commands) covering joint1-5
#     plus gripper_joint_1, so EasyTrainer can drive arm + gripper from
#     one topic.
...
def generate_launch_description():
    declared_arguments = [
        ...
        DeclareLaunchArgument(
            ...
            description='Port name for the Dynamixel USB serial bridge.',
        ),
        DeclareLaunchArgument(
            'namespace',
            default_value='',
