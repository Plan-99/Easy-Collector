...
- **실사용 구동(제4·관찰가능성)**: 앱은 backend script 를 직접 찌르지 말고 **사람처럼 UI 를 Playwright 로 조작**한다(센서·워크스페이스 등록 → 수집 → 학습 → 추론). 수집/학습 같은 **장시간 작업은 진행 창을 띄워(headed, 사람이 볼 수 있는 display) 두고**, 그 **같은 세션에서 주기적으로 스크린샷 + console-error 스캔**해 "잘 도는지 + UI 안 깨졌는지"를 실측한다. 보이지 않으면 사람이 진행을 감시할 수 없고, 무인 루프도 중간 회귀(렌더 깨짐/멈춤)를 놓친다. 스크린샷·에러는 결정적 평가자의 일부로 취급(자기보고 "잘 됨" 금지).
- **단일 세션 불변식(상태 비침해)**: 장시간 backend 프로세스(데이터 수집/학습)가 도는 동안 **새 브라우저/탭으로 앱을 열지 말 것**. 많은 SPA 는 onMount 에서 "stale 프로세스 정리"로 실행 중 작업을 stop 시킨다(예: EasyTrainer `App.vue` → `/stop_process` 가 `record_episode` 를 죽임). 그래서 모니터링·스크린샷은 *작업을 시작한 바로 그 페이지*에서 한다. 별도 detect 가 필요하면 작업이 끝난 뒤에.
...
- **headless 로만 돌려 "보이지 않는" 장시간 작업**: 사람이 감시 못 하고 렌더 멈춤/깨짐을 놓침 → 장시간 작업은 headed + 주기 스크린샷.
- **모니터링하려고 새 탭/브라우저 열기**: 실행 중인 수집/학습을 onMount 정리 로직이 죽일 수 있음 → 반드시 같은 세션에서.
