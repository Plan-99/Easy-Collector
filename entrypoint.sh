...
tmux send-keys -t ros_dev:0 "echo '--- Starting UI dev server ---' && cd /root/src/ui && npm run dev" C-m
...
tmux send-keys -t ros_dev:2 "echo '--- Starting backend API ---' && cd /root/src && python3 -m backend.api.app" C-m
