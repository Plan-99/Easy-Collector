...
def build_ACT_model_and_optimizer(args_overide):
    

    parser = argparse.ArgumentParser('DETR training and evaluation script', parents=[get_args_parser()])
    args = parser.parse_args(
        args = [
            '--ckpt_dir', args_overide['ckpt_dir'],
            '--policy_class', args_overide['policy_class'],
            '--task_name', args_overide['task_name'],
            '--seed', str(args_overide['seed']),
            # '--num_epochs', str(args_overide['num_epochs']),
        ]
    )

    for k, v in args_overide.items():
        setattr(args, k, v)
