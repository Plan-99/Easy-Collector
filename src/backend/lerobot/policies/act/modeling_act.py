...
class ACT(nn.Module):
    ...
    def forward(self, batch: dict[str, Tensor]) -> tuple[Tensor, tuple[Tensor, Tensor] | tuple[None, None]]:
        ...
        robot_pos = batch["observation.state"].reshape(batch_size, -1)  # (B, state_dim)
        ...
        if self.config.use_vae and "action" in batch:
            ...
            if self.config.robot_state_feature:
                robot_state_embed = self.vae_encoder_robot_state_input_proj(robot_pos)
        ...
        # Robot state token.
        if self.config.robot_state_feature:
            encoder_in_tokens.append(self.encoder_robot_state_input_proj(robot_pos))
