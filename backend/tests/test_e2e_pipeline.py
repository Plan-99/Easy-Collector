...
실행해 어디에서도 끊김/예외가 없는지 검증한다:
    ...
    7. ACT policy 행 생성 + checkpoint(num_epochs=100, batch_size=4) 생성
    8. /train/queue enqueue → 로컬 training_server(localhost:5100)에서 학습
    9. status='finished' 까지 polling
   10. /checkpoint/<id>/:start_test 로 추론 짧게 돌리고 정상 stop
   11. planner 생성 — joint_position(homepose) → timesleep → checkpoint(추론)
...
Usage:
    python -m backend.tests.test_e2e_pipeline                # default
    python -m backend.tests.test_e2e_pipeline --hz 10 --episodes 2 --train-epochs 100
    python -m backend.tests.test_e2e_pipeline --skip-train   # collection+inference만
"""
import argparse
import json
import sys
...
def _run_pipeline(api: Backend, args):
    ...
    if not args.skip_inference:
        with stage(f'run inference for {args.inference_seconds}s'):
            run_inference(api, checkpoint_id, workspace, robot,
                           run_seconds=args.inference_seconds)
    else:
        print('\n[INFO] --skip-inference 으로 추론 단계 생략')

    if args.skip_planner:
        print('\n[OK] inference finished; --skip-planner 이므로 종료')
        return
    ...
    with stage(f'run planner (joint_position → timesleep {args.planner_sleep_seconds}s '
               f'→ checkpoint {args.planner_checkpoint_seconds}s)'):
        run_planner(
    ...
    print('\n[OK] Easy Trainer end-to-end pipeline (incl. planner) completed without errors.')
