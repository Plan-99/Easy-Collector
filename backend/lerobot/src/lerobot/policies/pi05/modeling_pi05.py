...
class PaliGemmaWithExpertModel(
    ...
    def __init__(
        ...
        print(f'[PaliGemma] Precision conversion done. Setting requires_grad...', flush=True)
        self._set_requires_grad()
        print(f'[PaliGemma] Init complete.', flush=True)
...
class PI05Pytorch(nn.Module):  # see openpi `PI0Pytorch`
    ...
    def __init__(self, config: PI05Config, rtc_processor: RTCProcessor | None = None):
        ...
        print(f'[PI05Pytorch] Creating PaliGemmaWithExpertModel (precision={config.dtype})...', flush=True)
        ...
        print(f'[PI05Pytorch] PaliGemmaWithExpertModel created.', flush=True)
...
class PI05Policy(PreTrainedPolicy):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        print(f'[PI05] init_rtc_processor...', flush=True)
        self.init_rtc_processor()
        print(f'[PI05] Creating PI05Pytorch (dtype={config.dtype}, device={config.device})...', flush=True)
        self.model = PI05Pytorch(config, rtc_processor=self.rtc_processor)
        print(f'[PI05] PI05Pytorch created.', flush=True)

        # Enable gradient checkpointing if requested
        if config.gradient_checkpointing:
            self.model.gradient_checkpointing_enable()
            print(f'[PI05] Gradient checkpointing enabled.', flush=True)

        print(f'[PI05] Moving model to {config.device}...', flush=True)
        self.model.to(config.device)
        if torch.cuda.is_available():
            alloc = torch.cuda.memory_allocated() / 1024**3
            print(f'[PI05] Model on {config.device}. GPU alloc={alloc:.2f}GB', flush=True)
