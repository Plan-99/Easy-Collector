"""
Pick-and-Place 엔트리포인트.

Franka 로봇으로 오브젝트를 잡아서 목표 위치에 올려놓는 예제.

사용법 (MoveIt2 Docker 컨테이너 내부):
    ros2 run isaac_robot_control pick_and_place
    ros2 run isaac_robot_control pick_and_place --ros-args -p pick_object:=BlueCube -p place_target:=GreenPlate
"""

import rclpy

from isaac_robot_control.robots import FrankaConfig
...
def main(args=None):
    ...
    # ROS2 파라미터로 오브젝트 이름 설정 가능
    tmp_node = rclpy.create_node("_pick_and_place_params")
